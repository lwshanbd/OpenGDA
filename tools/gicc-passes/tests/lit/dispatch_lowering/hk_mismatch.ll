...
; error from GICCDispatchLowering — no host trace can be synthesized
; for an HK-incapable site.
;
; The kernel JSON pre-staged in metadir mimics what
; GICCHKAnalysis emits for a site whose 'size' arg depends on
; llvm.amdgcn.workitem.id.x: hk_capable=false plus a hk_fail_reason.
;
; RUN: rm -rf %t.metadir && mkdir -p %t.metadir
; RUN: cp %S/../Inputs/k_one_put_hk_fail_meta.json \
; RUN:    %t.metadir/_Z9k_one_putPN4gicc9DeviceCtxEim.json
; RUN: not env GICC_MODE=lower GICC_META_DIR=%t.metadir \
; RUN:     GICC_HINT_IN=%S/../Inputs/hint_dwq_for_k_one_put.json \
...
; CHECK: only CPU_PROXY_ENQUEUE accepts non-HK args
