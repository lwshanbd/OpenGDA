...
std::string TraceEmitter::ensure_sidecar(const std::string& main_file) {
    ...
    if (!opened_sidecars_.insert(sidecar).second) {
        return sidecar; // Already opened this TU; nothing to do.
    }
    ...
    out << "// No GICC kernels lifted in " << main_file << "\n";
    return sidecar;
}
...
std::string TraceEmitter::emit(const KernelInfo& ki, const HKAnalysis& hk) {
    ...
    if (first_open) {
        ...
        // defines as the main TU; Phase G arranges that automatically.
        out << "// AUTO-GENERATED by gicc-clang-plugin. DO NOT EDIT.\n";
