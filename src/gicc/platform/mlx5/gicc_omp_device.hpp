 ...
 * NIC doorbell (mlx5_device.hpp). `lane` picks one of the per-peer QPs, so
...
#include "gicc/platform/mlx5/mlx5_device.hpp"   // declare-target when -fopenmp
#include <cstddef>
#include <cstdint>
...
inline void put_signal(gicc::DeviceCtx* ctx, int target_rank,
    ...
    if (!ctx) return;
    gicc::mlx5::dev::put_signal(ctx, target_rank, dst_buf, dst_offset,
...
inline void get(gicc::DeviceCtx* ctx, int source_rank,
                ...
                size_t size, int lane = 0) {
    if (!ctx) return;
    gicc::mlx5::dev::get(ctx, source_rank, src_buf, src_offset,
