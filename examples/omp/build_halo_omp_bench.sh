#!/bin/bash
# build_halo_omp_bench.sh - GICC-from-OpenMP halo benchmark: OpenMP app TU + HIP bridge + HIP proxy/runtime.
set -euo pipefail
...
SCRATCH="${GICC_ROOT}/build_ofi/omp_halo_bench_scratch"
mkdir -p "${SCRATCH}"
...
"${HIPCC}" -fopenmp --offload-arch=gfx90a -O3 -std=gnu++17 \
    ...
    -c "${GICC_ROOT}/examples/omp/halo_omp_bench.cpp" -o "${OBJ_DIR}/halo_omp_bench.o"
...
"${HIPCC}" -fopenmp --offload-arch=gfx90a \
    ...
    "${OBJ_DIR}/halo_omp_bench.o" "${OBJ_DIR}/bridge.o" \
    "${OBJ_DIR}/runtime_helpers.o" "${OBJ_DIR}/proxy_thread.o" "${OBJ_DIR}/proxy_libfabric.o" \
    -o "${GICC_ROOT}/build_ofi/halo_omp_bench" \
...
set +x
echo "BUILT: ${GICC_ROOT}/build_ofi/halo_omp_bench"
