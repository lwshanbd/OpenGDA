#!/bin/bash
# build_e4.sh - E4 get() RMA-read correctness: OpenMP app TU + HIP bridge + HIP proxy/runtime.
set -euo pipefail
...
SCRATCH="${GICC_ROOT}/build_ofi/omp_e4_scratch"
mkdir -p "${SCRATCH}"
...
"${HIPCC}" -fopenmp --offload-arch=gfx90a -O3 -std=gnu++17 \
    ...
    -c "${GICC_ROOT}/examples/omp/e4_get.cpp" -o "${OBJ_DIR}/e4.o"
...
set +x
echo "BUILT: ${GICC_ROOT}/build_ofi/e4_get"
