#!/bin/bash
# Build the common MM source as GiOMP, DiOMP, and GPU-aware MPI with the same
# clang-21 OpenMP-offload compiler.  GiOMP uses its clang-21 runtime build so
# compiler differences do not contaminate the three-way compute comparison.
set -euo pipefail
...
GIOMP_LIB="${GIOMP_MM_LIBDIR:-${GICC_ROOT}/build_ofi/lib-clang21-mm}"
OUT="${GIOMP_MM_OUTDIR:-${GICC_ROOT}/build_ofi/mm_eval}"
SRC="${GICC_ROOT}/examples/omp/giomp_mm_eval.cpp"
...
GIOMP_MPI_GTL="${MPI_GTL}" GIOMP_LIBDIR="${GIOMP_LIB}" \
    bash "${GICC_ROOT}/examples/omp/build_giomp_example.sh" \
        "${SRC}" "${OUT}/giomp_mm_eval"

COMMON=(
    -O3 -std=gnu++17 -fopenmp --offload-arch=gfx90a --rocm-path="${ROCM}"
    -Wno-pass-failed -I"${ROCM}/include" -isystem "${MPI}/include"
)
COMMON_LINK=(
...
set -x
"${CXX}" "${COMMON[@]}" -DMM_BACKEND_MPI=1 "${SRC}" \
    "${COMMON_LINK[@]}" -o "${OUT}/mpi_mm_eval"

"${CXX}" -O3 -std=gnu++17 -fopenmp --offload-arch=gfx90a \
    --rocm-path="${ROCM}" -Wno-pass-failed \
    -DMM_BACKEND_DIOMP=1 -DDIOMP_ENABLE_HIP -D__HIP_PLATFORM_AMD__ \
    ...
    "${SRC}" \
    -L"${DIOMP_INSTALL}/lib/x86_64-unknown-linux-gnu" \
    -L"${DIOMP_INSTALL}/lib" -ldiomp "${ROCM}/lib/librccl.so" \
    -L"${GASNET_ROOT}/lib" -lgasnet-ofi-par \
...
echo "BUILT: ${OUT}/giomp_mm_eval"
echo "BUILT: ${OUT}/diomp_mm_eval"
echo "BUILT: ${OUT}/mpi_mm_eval"
