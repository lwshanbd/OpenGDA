#!/bin/bash
# build_e1.sh - E1 single-put: OpenMP app TU + HIP bridge + HIP proxy/runtime.
set -euo pipefail
...
SCRATCH="${GICC_ROOT}/build_ofi/omp_e1_scratch"
mkdir -p "${SCRATCH}"
...
"${HIPCC}" -fopenmp --offload-arch=gfx90a -O3 -std=gnu++17 \
    ...
    -c "${GICC_ROOT}/examples/omp/e1_single_put.cpp" -o "${OBJ_DIR}/e1.o"
...
set +x
echo "BUILT: ${GICC_ROOT}/build_ofi/e1_single_put"
