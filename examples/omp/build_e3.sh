#!/bin/bash
# build_e3.sh - E3 ring halo exchange from omp target region: OpenMP app TU + HIP bridge + HIP proxy/runtime.
set -euo pipefail
...
SCRATCH="${GICC_ROOT}/build_ofi/omp_e3_scratch"
mkdir -p "${SCRATCH}"
...
"${HIPCC}" -fopenmp --offload-arch=gfx90a -O3 -std=gnu++17 \
    ...
    -c "${GICC_ROOT}/examples/omp/e3_halo.cpp" -o "${OBJ_DIR}/e3.o"
...
set +x
echo "BUILT: ${GICC_ROOT}/build_ofi/e3_halo"
