#!/bin/bash
# Build halo_face_sweep microbench. No LTO pass needed (direct device-side
...
"${HIPCC}" "${CFLAGS[@]}" -x hip -c \
    "${GICC_ROOT}/examples/proxy/halo_face_sweep.cpp" \
    -o "${OBJ_DIR}/bench.o"
...
"${HIPCC}" -O3 --offload-arch=gfx90a --hip-link \
    ...
    -o "${GICC_ROOT}/build_ofi/halo_face_sweep" \
...
set +x
echo "BUILT: ${GICC_ROOT}/build_ofi/halo_face_sweep"
