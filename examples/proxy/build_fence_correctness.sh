#!/bin/bash
# Build fence_correctness microbench. No LTO pass needed (direct device-side
...
"${HIPCC}" "${CFLAGS[@]}" -x hip -c \
    "${GICC_ROOT}/examples/proxy/fence_correctness.cpp" \
    -o "${OBJ_DIR}/bench.o"
...
"${HIPCC}" -O3 --offload-arch=gfx90a --hip-link \
    ...
    -o "${GICC_ROOT}/build_ofi/fence_correctness" \
...
set +x
echo "BUILT: ${GICC_ROOT}/build_ofi/fence_correctness"
