#!/bin/bash
# Build rule_microbench microbench. No LTO pass needed (direct device-side
...
"${HIPCC}" "${CFLAGS[@]}" -x hip -c \
    "${GICC_ROOT}/examples/proxy/rule_microbench.cpp" \
    -o "${OBJ_DIR}/bench.o"
...
"${HIPCC}" -O3 --offload-arch=gfx90a --hip-link \
    ...
    -o "${GICC_ROOT}/build_ofi/rule_microbench" \
...
set +x
echo "BUILT: ${GICC_ROOT}/build_ofi/rule_microbench"
