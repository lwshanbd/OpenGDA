#!/bin/bash
# Build ipc_copy_sweep microbench. No LTO pass needed (direct device-side
...
"${HIPCC}" "${CFLAGS[@]}" -x hip -c \
    "${GICC_ROOT}/examples/proxy/ipc_copy_sweep.cpp" \
    -o "${OBJ_DIR}/bench.o"
...
"${HIPCC}" -O3 --offload-arch=gfx90a --hip-link \
    ...
    -o "${GICC_ROOT}/build_ofi/ipc_copy_sweep" \
...
set +x
echo "BUILT: ${GICC_ROOT}/build_ofi/ipc_copy_sweep"
