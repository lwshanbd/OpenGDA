#!/bin/bash
# Build halo_bench microbench. No LTO pass needed (direct device-side
...
"${HIPCC}" "${CFLAGS[@]}" -x hip -c \
    "${GICC_ROOT}/examples/proxy/halo_bench.cpp" \
    -o "${OBJ_DIR}/bench.o"
...
"${HIPCC}" -O3 --offload-arch=gfx90a --hip-link \
    ...
    -o "${GICC_ROOT}/build_ofi/halo_bench" \
...
set +x
echo "BUILT: ${GICC_ROOT}/build_ofi/halo_bench"
