#!/bin/bash
# Build bench_pingpong microbench. No LTO pass needed (direct device-side
...
"${HIPCC}" "${CFLAGS[@]}" -x hip -c \
    "${GICC_ROOT}/examples/proxy/bench_pingpong.cpp" \
    -o "${OBJ_DIR}/bench.o"
...
"${HIPCC}" -O3 --offload-arch=gfx90a --hip-link \
    ...
    -o "${GICC_ROOT}/build_ofi/bench_pingpong" \
...
set +x
echo "BUILT: ${GICC_ROOT}/build_ofi/bench_pingpong"
