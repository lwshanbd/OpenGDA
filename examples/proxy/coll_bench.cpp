...
int main(int argc, char** argv) {
    ...
    // ---- COOPERATIVE fused all-reduce (full-GPU reduction, single kernel) ----
    for (int si = 0; si < n_sizes; ++si) {
        const int chunk = chunks[si];
        const int count = N * chunk;
        std::vector<float> hv(count);
        ...
        MPI_Reduce(&errs, &all_errs, 1, MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);
        if (rank == 0)
            printf("[coop allreduce correctness @%zu B: %s, %d total errors]\n",
                   (size_t)count * sizeof(float),
                   all_errs == 0 ? "PASS" : "FAIL", all_errs);
    }
    for (int si = 0; si < n_sizes; ++si) {
    ...
    // ---- LOCALITY-AWARE coop (xGMI intra-node, proxy cross-node) ----
    for (int si = 0; si < n_sizes; ++si) {
        const int chunk = chunks[si];
        const int count = N * chunk;
        std::vector<float> hv(count);
        ...
        (void)hipMemcpy(d_fz, hv.data(), (size_t)count * sizeof(float),
                        hipMemcpyHostToDevice);
        (void)hipDeviceSynchronize();
        gicc_coll::ring_allreduce_coop_loc(rt, fz_buf, d_fz, fz_recv, d_fz_recv,
                                           flag_buf, d_flag, one_buf, chunk);
        (void)hipMemcpy(hv.data(), d_fz, (size_t)count * sizeof(float),
                        hipMemcpyDeviceToHost);
        (void)hipDeviceSynchronize();
        ...
        MPI_Reduce(&errs, &all_errs, 1, MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);
        if (rank == 0)
            printf("[loc allreduce correctness @%zu B: %s, %d total errors]\n",
                   (size_t)count * sizeof(float),
                   all_errs == 0 ? "PASS" : "FAIL", all_errs);
    }
    for (int si = 0; si < n_sizes; ++si) {
        ...
        if (rank == 0)
            printf("%-10s %12zu | %14.2f %14.2f %7.2fx\n",
                   "ar-loc", arr_bytes, gmax, mmax, mmax / gmax);
    }
