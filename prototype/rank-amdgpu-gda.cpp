  ...
  for (int size_idx = 0; size_idx < num_test_sizes; size_idx++) {
    ...
    for (int iter = 0; iter < NUM_ITERATIONS; iter++) {
      ...
      if (myrank == 0) {
        ...
        if (ret) {
          fprintf(stderr,
                  "Rank %d: fi_control(FI_QUEUE_WORK) failed: %s (%d)\n",
                  myrank, fi_strerror(-ret), ret);
          if (ret == -FI_ENOSYS) {
            fprintf(stderr, "Rank %d: DWQ not supported by provider!\n",
                    myrank);
          }
          continue;
        }
        ...
        do {
          ...
          if (iter == 0 && size_idx == 0 && poll_count % 10000 == 0) {
              uint64_t trigger_current = fi_cntr_read(trigger_cntr);
              fprintf(stderr, "Rank %d: Poll %dK - trigger=%lu, completion=%lu, target=%lu\n",
                     myrank, poll_count/1000, trigger_current, completion_val, target_completion);
          }
