 ...
 */
 ...
 #define CHECK_HIP(x, msg) do { \
     hipError_t err = (x); \
     if (err != hipSuccess) { \
         fprintf(stderr, "Rank %d: %s failed: %s (%d)\n", myrank, msg, hipGetErrorString(err), err); \
         PMI2_Finalize(); \
         exit(1); \
     } \
 } while(0)
 ...
 int main(void) {
     ...
     CHECK(register_memory_region(domain, ep, cxi_info, d_local_buf, max_buf_size, &mr_local, true, device),
           "register_memory_region(local GPU)");
 
     struct fid_mr *mr_remote;
     CHECK(register_memory_region(domain, ep, cxi_info, d_remote_buf, max_buf_size, &mr_remote, true, device),
     ...
     if (myrank == 0) {
         printf("\n");
         printf("========================================\n");
         printf("GPU-to-GPU RDMA Latency Benchmark\n");
         printf("========================================\n");
     ...
     for (int size_idx = 0; size_idx < num_test_sizes; size_idx++) {
         ...
         if (myrank == 0) {
             ret = fi_write(ep, d_local_buf, current_size, desc_local,
                           peer_fi_addr, remote_addr_for_rma, peer_remote_key, NULL);
 
             struct fi_cq_data_entry cqe;
             while (fi_cq_read(cq, &cqe, 1) == -FI_EAGAIN);
         }
 
         // Timed iterations - no sync, pure RDMA write performance
